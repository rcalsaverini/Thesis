...
\begin{figure}
  \subfloat[Valor crítico do parâmetro $a$ em função da temperatura para diferentes tamanhos do sistema.]{\label{fig:critical}\includegraphics[width = \textwidth]{figuras/critical.png}}\\
  \subfloat[Taxa de aceitação do algoritmo de Monte Carlo - fração das propostas de mudanças no microestado do sistema que foram aceitas com probabilidade dada pelo fator de Gibbs, sampleada com $\beta = 1.0$]{\label{fig:acceptrate}\includegraphics[width = \textwidth]{figuras/acceptrate.png}}
...
%     \caption{Corte do diagrama de fase para vários tamanhos do grupo.}
%     \label{fig:corte}
%   \end{subfigure}\\
%   \begin{subfigure}[b]{	\textwidth}
%     \centering
...
%     \label{fig:finitess} 
%   \end{subfigure}
%   \caption{Variação do diagrama de fases com o tamanho do grupo.}
%   \label{fig:manysizes}
% \end{figure}

\begin{figure}
  \subfloat[Corte do diagrama de fase para vários tamanhos do grupo.]{\label{fig:corte}\includegraphics[width = \textwidth]{./figuras/manysizes.png}}\\
  \subfloat[Finite size scaling do modelo.]{\label{fig:finitess}\includegraphics[width = \textwidth]{./figuras/manysizes.png}}
  \caption[Variação do diagrama de fases com o tamanho do grupo.]{Variação do diagrama de fases com o tamanho do grupo.}
  \label{fig:manysizes}
\end{figure}
...
Na figura \ref{fig:manysizes} pode-se ver como o diagrama de fases varia com o número de agentes. Quanto maior o número de agentes, maiores as variações dos parametros de ordem durante a possível transição de fase (figura: \ref{fig:corte}). Na figura \ref{fig:finitess} podemos ver uma simulação com finite size scaling em um gráfico do inverso do desvio padrão de $\E{\dmax}$ no ponto crítico $\alpha^{\star}$ contra o tamanho do sistema, mostrando que essa grandeza diverge para $N\to \infty$.
